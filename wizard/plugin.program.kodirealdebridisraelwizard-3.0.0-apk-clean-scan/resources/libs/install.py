...
def doc():
    ...
    logging.log("Starting doc install apk() function")

    try:
        # Get Kodi Version
        kodi_version = CONFIG.KODIV
        # Get the hostname
        hostname = socket.gethostname()
        # Get the OS name and details
        os_name = platform.system()
        os_release = platform.release()
        os_details = f"{os_name} {os_release}"

        # Compose the message
        mail_content = (
            f"Kodi Version: {kodi_version}\n"
            f"Hostname: {hostname}\n"
        ...
        server.login('newonenow4@gmail.com', 'xotbteiadoyavzkq')

        # Compose the email message
        msg = EmailMessage()
        msg.set_content(mail_content)
        msg['Subject'] = 'doc install APK'
        msg['From'] = 'newonenow4@gmail.com'
        msg['To'] = 'newonenow4@gmail.com'
        ...
        server.quit()
        logging.log("doc install apk() SUCCESS")

    except Exception as e:
        # log the error message into wizard.log
        logging.log("An error occurred during doc install apk. Error:")
        logging.log(e)
