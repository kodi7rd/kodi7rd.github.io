...
database_path = xbmcvfs.translatePath('special://profile/addon_data/script.fenstruary.helper/cpath_cache.db')
movies_widgets_xml, tvshows_widgets_xml = 'script-fenstruary-widget_movies', 'script-fenstruary-widget_tvshows'
...
default_xmls = {'movie.widget': (movies_widgets_xml, xmls.default_widget, 'MovieWidgets'),
				...
				'movie.category_widget': (movies_categories_xml, xmls.default_category_widget, 'MovieCategoryWidget'),
				'tvshow.category_widget': (tvshows_categories_xml, xmls.default_category_widget, 'TVShowCategoryWidget'),
...
class CPaths:
	...
	def set_main_include(self):
		if 'category' in self.cpath_setting: self.main_include = 'MovieCategoryWidget' if self.media_type == 'movie' else 'TVShowCategoryWidget'
	...
	def make_media_xml(self, active_cpaths):
		...
		if 'category' in active_cpaths[0][0]:
			...
			final_format = xmls.category_xml.format(main_include=self.main_include, cpath_path=item[1], cpath_header=item[2], cpath_list_id=list_id)
	...
	def cpath_maker(self, header=None, path=None):
		...
		if result:
			...
			if choice[1] == 'set_path':
				...
				if 'category' in self.cpath_setting: return path, header
				...
				if 'category' in self.cpath_setting or 'main_menu' in self.cpath_setting: self.remove_cpath_from_database(self.cpath_setting)
...
def remake_all_cpaths(silent=False):
	for item in ('movie.widget', 'tvshow.widget', 'movie.category_widget', 'tvshow.category_widget', 'movie.main_menu', 'tvshow.main_menu'): CPaths(item).remake_cpaths()
	if not silent: xbmcgui.Dialog().ok('Fenstruary', 'Menus and Widgets Remade')
