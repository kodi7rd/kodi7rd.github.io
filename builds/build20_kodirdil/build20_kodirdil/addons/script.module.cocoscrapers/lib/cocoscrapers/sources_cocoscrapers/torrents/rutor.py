# -*- coding: utf-8 -*-
# created by Venom for Fenomscrapers (1-18-2024)
"""
...
#Foreign TV = 0/4/110/8
	...
	def sources(self, data, hostDict):
		...
		try:
			...
			if 'tvshowtitle' in data:
				...
				url_insert = '0/4/110/8'
			else:
				title = data['title'].replace('&', 'and').replace('/', ' ').replace('$', 's')
				episode_title = None
				hdlr = year
				url_insert = '0/1/110/8'
			query = '%s %s' % (re.sub(r'[^A-Za-z0-9\s\.-]+', '', title), hdlr)
			url = '%s%s' % (self.base_link, self.search_link % (url_insert, quote_plus(query)))
		...
		for row in rows:
			try:
				...
				# log_utils.log('hash = %s' % hash)

				name = re.search(r'/torrent/.+?["\']>(.+?)<', columns[1], re.I).group(1)
				name = source_utils.clean_name(name).replace('2./.', '').replace('./.', '').replace('|.', '').replace('...', '.').replace('..', '.').replace(',.', '.')
				url = '%s%s' % (url, name)
				...
				try:
					...
					# log_utils.log('seeders = %s' % seeders)
	...
	def sources_packs(self, data, hostDict, search_series=False, total_seasons=None, bypass_filter=False):
		...
		try:
			...
			if search_series:
				queries = [
						self.search_link % ('0/4/110/8', quote_plus(query + ' Season')),
						self.search_link % ('0/4/110/8', quote_plus(query + ' Complete'))]
			else:
				queries = [
						self.search_link % ('0/4/110/8', quote_plus(query + ' S%s' % self.season_xx)),
						self.search_link % ('0/4/110/8', quote_plus(query + ' Season %s' % self.season_x))]
			threads = []
