...
xbmc_actor, set_category, get_property, get_playback_int = kodi_utils.xbmc_actor, kodi_utils.set_category, kodi_utils.get_property, kodi_utils.get_playback_int
	...
	def _process():
		for item in episodes_data:
			try:
				...
				url_params = build_url({'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode, 'playback_int': playback_int})
				...
				set_properties({'twilight.extras_params': extras_params, 'twilight.options_params': options_params, 'twilight.unwatched_params': unwatched_params,
								'twilight.watched_params': watched_params, 'twilight.clearprog_params': clearprog_params, 'episode_type': episode_type})
	...
	def _process(_position, ep_data):
		try:
			...
			if list_type_starts_with('next_'):
				...
				if nextep_include_airdate: airdate = '[COLOR %s][%s][/COLOR] ' % (twilight_highlight, display_premiered)
				else: airdate = ''
				...
				if unaired:
					...
					display = '[COLOR %s]%s][/COLOR]%s' % (twilight_highlight, displays[0], displays[1])
			else:
				color_tags = ('[COLOR %s]' % twilight_highlight, '[/COLOR]') if unaired else ('', '')
