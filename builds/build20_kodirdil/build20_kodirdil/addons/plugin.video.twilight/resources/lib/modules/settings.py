...
download_directories_dict = {'movie': 'twilight.movie_download_directory', 'episode': 'twilight.tvshow_download_directory', 'thumb_url': 'twilight.image_download_directory',
							'image_url': 'twilight.image_download_directory','image': 'twilight.image_download_directory', 'premium': 'twilight.premium_download_directory',
							None: 'twilight.premium_download_directory', 'None': False}
...
paginate_dict = {True: 'twilight.paginate.limit_widgets', False: 'twilight.paginate.limit_addon'}
prescrape_scrapers_tuple = ('furk', 'easynews', 'rd_cloud', 'pm_cloud', 'ad_cloud', 'folders')
sort_to_top_dict = {'folders': 'twilight.results.sort_folders_first', 'rd_cloud': 'twilight.results.sort_rdcloud_first',
					'pm_cloud': 'twilight.results.sort_pmcloud_first', 'ad_cloud': 'twilight.results.sort_adcloud_first'}
...
def generic_list_sorting(setting_id):
	return get_setting('twilight.sort.%s' % setting_id, 'popularity.desc').replace('&amp;', '&')
...
def use_skin_fonts():
	return get_setting('twilight.use_skin_fonts')

def use_custom_skins():
	return get_setting('twilight.use_custom_skins', 'true') == 'true'

def results_format():
	return str(get_setting('twilight.results.list_format', 'List').lower())

def results_style():
	return results_style_dict[get_setting('twilight.results.use_contrast', 'true')]
...
def store_resolved_torrent_to_cloud(debrid_service, pack):
	setting_value = int(get_setting('twilight.store_resolved_torrent.%s' % debrid_service.lower(), '0'))
	return setting_value in (1, 2) if pack else setting_value == 1

def enabled_debrids_check(debrid_service):
	if not get_setting('twilight.%s.enabled' % debrid_service) == 'true': return False
	return authorized_debrid_check(debrid_service)

def authorized_debrid_check(debrid_service):
	if get_setting('twilight.%s.token' % debrid_service) in (None, ''): return False
	return True

def check_premium_account_status():
	return get_setting('twilight.check_premium_account_status', 'false') == 'true'

def playback_settings():
	return (int(get_setting('twilight.playback.watched_percent', '90')), int(get_setting('twilight.playback.resume_percent', '5')))

def monitor_playback():
	return get_setting('twilight.playback.monitor_success', 'true') == 'true'

def playback_attempt_pause():
	return int(get_setting('twilight.playback.pause_start', '1500'))

def limit_resolve():
	return get_setting('twilight.playback.limit_resolve', 'false') == 'true'

def disable_content_lookup():
	return get_setting('twilight.playback.disable_content_lookup') == 'true'

def easynews_max_retries():
	return int(get_setting('twilight.playback.easynews_max_retries', '1'))

def show_unaired_watchlist():
	return get_setting('twilight.show_unaired_watchlist', 'false') == 'true'
	
def movies_directory():
	return translate_path(get_setting('twilight.movies_directory'))
	
def tv_show_directory():
	return translate_path(get_setting('twilight.tv_shows_directory'))
...
def source_folders_directory(media_type, source):
	setting = 'twilight.%s.movies_directory' % source if media_type == 'movie' else 'twilight.%s.tv_shows_directory' % source
	if get_setting(setting) not in ('', 'None', None): return translate_path( get_setting(setting))
	else: return False

def avoid_episode_spoilers():
	return get_setting('twilight.avoid_episode_spoilers', 'false') == 'true'

def paginate(is_home):
	paginate_lists = int(get_setting('twilight.paginate.lists', '0'))
...
def jump_to_enabled():
	return int(get_setting('twilight.paginate.jump_to', '0'))

def use_year_in_search():
	return get_setting('twilight.meta_use_year_in_search') == 'true'

def quality_filter(setting):
	return get_setting('twilight.%s' % setting).split(', ')

def audio_filters():
	setting = get_setting('twilight.filter_audio')
	if setting in ('', None): return []
	return setting.split(', ')

def include_prerelease_results():
	return get_setting('twilight.include_prerelease_results', 'true') == 'true'

def auto_play(media_type):
	return get_setting('twilight.auto_play_%s' % media_type, 'false') == 'true'

def autoplay_next_episode():
	if auto_play('episode') and get_setting('twilight.autoplay_next_episode', 'false') == 'true': return True
	else: return False

def autoscrape_next_episode():
	if not auto_play('episode') and get_setting('twilight.autoscrape_next_episode', 'false') == 'true': return True
	else: return False

def auto_rescrape_with_all():
	return get_setting('twilight.results.autorescrape_with_all', 'false') == 'true'

def auto_nextep_settings(play_type):
	play_type = 'autoplay' if play_type == 'autoplay_nextep' else 'autoscrape'
	window_percentage = 100 - int(get_setting('twilight.%s_next_window_percentage' % play_type, '95'))
	use_chapters = get_setting('twilight.%s_use_chapters' % play_type, 'true') == 'true'
	scraper_time = int(get_setting('twilight.results.timeout', '60')) + 20
	if play_type == 'autoplay':
		alert_method = int(get_setting('twilight.autoplay_alert_method', '0'))
		default_action = default_action_dict[get_setting('twilight.autoplay_default_action', '1')] if alert_method == 0 else 'cancel'
...
def filter_status(filter_type):
	return int(get_setting('twilight.filter_%s' % filter_type, '0'))

def ignore_results_filter():
	return int(get_setting('twilight.results.ignore_filter', '0'))

def display_uncached_torrents():
	return get_setting('twilight.torrent.display.uncached', 'false') == 'true'

def trakt_sync_interval():
	setting = get_setting('twilight.trakt.sync_interval', '25')
	interval = int(setting) * 60
	return setting, interval

def trakt_sync_refresh_widgets():
	return get_setting('twilight.trakt.sync_refresh_widgets', 'true') == 'true'

def lists_sort_order(setting):
	return int(get_setting('twilight.sort.%s' % setting, '0'))

def auto_start_twilight():
	return get_setting('twilight.auto_start_twilight', 'false') == 'true'

def furk_active():
	if get_setting('twilight.provider.furk', 'false') == 'true':
		if not get_setting('twilight.furk_api_key'):
			furk_status = False if '' in (get_setting('twilight.furk_login'), get_setting('twilight.furk_password')) else True
		else: furk_status = True
	else: furk_status = False
	return furk_status

def easynews_active():
	if get_setting('twilight.provider.easynews', 'false') == 'true':
		easynews_status = False if '' in (get_setting('twilight.easynews_user'), get_setting('twilight.easynews_password')) else True
	else: easynews_status = False
	return easynews_status

def tv_progress_location():
	return int(get_setting('twilight.tv_progress_location', '0'))

def extras_enable_extra_ratings():
	return get_setting('twilight.extras.enable_extra_ratings', 'true') == 'true'

def extras_enabled_ratings():
	return get_setting('twilight.extras.enabled_ratings', 'Meta, Tom/Critic, Tom/User, IMDb, TMDb').split(', ')

def extras_enable_scrollbars():
	return get_setting('twilight.extras.enable_scrollbars', 'true')

def extras_exclude_non_acting():
	return get_setting('twilight.extras.exclude_non_acting_roles', 'true') == 'true'

def extras_windowed_playback():
	return get_setting('twilight.extras.windowed_playback', 'false') == 'true'

def extras_enabled_menus():
	setting = get_setting('twilight.extras.enabled', '2000,2050,2051,2052,2053,2054,2055,2056,2057,2058,2059,2060,2061,2062,2063')
	if setting in ('', None, 'noop', []): return []
	return [int(i) for i in setting.split(',')]

def check_prescrape_sources(scraper, media_type):
	if scraper in prescrape_scrapers_tuple: return get_setting('twilight.check.%s' % scraper) == 'true'
	if get_setting('twilight.check.%s' % scraper) == 'true' and auto_play(media_type): return True
	else: return False

def external_scraper_info():
	module = get_setting('twilight.external_scraper.module', None)
	if module: return module, module.split('.')[-1]
	else: return None, ''

def filter_by_name(scraper):
	if get_property('fs_filterless_search') == 'true': return False
	return get_setting('twilight.%s.title_filter' % scraper, 'false') == 'true'

def easynews_language_filter():
	enabled = get_setting('twilight.easynews.filter_lang') == 'true'
	if enabled: filters = get_setting('twilight.easynews.lang_filters').split(', ')
	else: filters = []
	return enabled, filters

def results_sort_order():
	sort_direction = -1 if get_setting('twilight.results.size_sort_direction') == '0' else 1
	return (
			...
			)[int(get_setting('twilight.results.sort_order', '1'))]
...
def active_internal_scrapers():
	...
	active = [i.split('.')[1] for i in settings if get_setting('twilight.%s' % i) == 'true']
	return active

def provider_sort_ranks():
	fu_priority = int(get_setting('twilight.fu.priority', '6'))
	en_priority = int(get_setting('twilight.en.priority', '7'))
	rd_priority = int(get_setting('twilight.rd.priority', '8'))
	ad_priority = int(get_setting('twilight.ad.priority', '9'))
	pm_priority = int(get_setting('twilight.pm.priority', '10'))
...
def auto_resume(media_type):
	auto_resume = get_setting('twilight.auto_resume_%s' % media_type)
	if auto_resume == '1': return True
	if auto_resume == '2' and auto_play(media_type): return True
	else: return False

def scraping_settings():
	highlight_type = int(get_setting('twilight.highlight.type', '0'))
	if highlight_type == 3:
	...
	if highlight_type in (0, 1):
		...
		debrid_cloud_highlight = get_setting('twilight.provider.debrid_cloud_colour', 'FF7A01CC')
		folders_highlight = get_setting('twilight.provider.folders_colour', 'FFB36B00')
		if highlight_type == 0:
			hoster_highlight = get_setting('twilight.hoster.identify', 'FF0166FF')
			torrent_highlight = get_setting('twilight.torrent.identify', 'FFFF00FE')
		else:
			rd_highlight = get_setting('twilight.provider.rd_colour', 'FF3C9900')
			pm_highlight = get_setting('twilight.provider.pm_colour', 'FFFF3300')
			ad_highlight = get_setting('twilight.provider.ad_colour', 'FFE6B800')
	elif highlight_type == 2:
		highlight_4K = get_setting('twilight.scraper_4k_highlight', 'FFFF00FE')
...
def omdb_api_key():
	return get_setting('twilight.omdb_api', '')
...
def default_all_episodes():
	return int(get_setting('twilight.default_all_episodes'))

def max_threads():
	if not get_setting('twilight.limit_concurrent_threads', 'false') == 'true': return 60
	return int(get_setting('twilight.max_threads', '60'))

def widget_hide_next_page():
	return get_setting('twilight.widget_hide_next_page', 'false') == 'true'

def show_unaired():
	return get_setting('twilight.show_unaired', 'true') == 'true'

def use_season_title():
	return get_setting('twilight.use_season_title', 'true') == 'true'

def show_specials():
	return get_setting('twilight.show_specials', 'false') == 'true'

def calendar_sort_order():
	return int(get_setting('twilight.trakt.calendar_sort_order', '0'))

def ignore_articles():
	return get_setting('twilight.ignore_articles', 'false') == 'true'

def date_offset():
	return int(get_setting('twilight.datetime.offset', '0')) + 5

def single_ep_display_title():
	return int(get_setting('twilight.single_ep_display', '0'))

def single_ep_format():
	return single_ep_format_dict[get_setting('twilight.single_ep_format', '1')]

def include_year_in_title(media_type):
	return int(get_setting('twilight.include_year_in_title', '0')) in year_in_title_dict[media_type]

def extras_open_action(media_type):
	return int(get_setting('twilight.extras.open_action', '0')) in extras_open_action_dict[media_type]

def tmdb_api_key():
	return get_setting('twilight.tmdb_api', tmdb_default_api)

def get_fanart_data():
	return get_setting('twilight.get_fanart_data') == 'true'

def get_resolution():
	return resolution_tuple[int(get_setting('twilight.image_resolutions', '2'))]

def get_language():
	return get_setting('twilight.meta_language', 'en')

def get_meta_filter():
	return get_setting('twilight.meta_filter', 'true')

def get_mpaa_region():
	return get_setting('twilight.meta_mpaa_region', 'US')

def widget_hide_watched():
	return get_setting('twilight.widget_hide_watched', 'false') == 'true'

def fanarttv_client_key():
	return get_setting('twilight.fanart_client_key', fanarttv_default_api)

def watched_indicators():
	if get_setting('twilight.trakt.user') == '': return 0
	return int(get_setting('twilight.watched_indicators', '0'))

def fanarttv_default():
	return get_setting('twilight.fanarttv.default', 'false') == 'true'

def nextep_display_settings():
	include_airdate = get_setting('twilight.nextep.include_airdate', 'false') == 'true'
	return {'unaired_color': 'red', 'unwatched_color': 'darkgoldenrod', 'include_airdate': include_airdate}

def nextep_content_settings():
	sort_type = int(get_setting('twilight.nextep.sort_type', '0'))
	sort_order = int(get_setting('twilight.nextep.sort_order', '0'))
	sort_direction = sort_order == 0
	sort_key = 'last_played' if sort_type == 0 else 'first_aired' if sort_type == 1 else 'name'
	include_unaired = get_setting('twilight.nextep.include_unaired', 'false') == 'true'
	include_unwatched = int(get_setting('twilight.nextep.include_unwatched', '0'))
