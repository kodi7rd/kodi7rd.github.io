...
twilight_str, window_top_str, listitem_property_str = ls(32036), 'Window.IsTopMost(%s)', 'ListItem.Property(%s)'
...
class DatabaseMaintenance:
	def run(self):
		...
		if due_clean == 0:
			...
			return logger(twilight_str, 'Database Maintenance Service First Run - Skipping')
		if current_time >= due_clean:
			clean_databases(current_time, database_check=False, silent=True)
			next_clean = str(int(self._get_timestamp(time + datetime.timedelta(days=3))))
			set_setting('database.maintenance.due', next_clean)
			return logger(twilight_str, 'Database Maintenance Service Finished')
		else: return logger(twilight_str, 'Database Maintenance Service Finished - Not Run')
...
class FirstRunActions:
	def run(self):
		logger(twilight_str, 'FirstRunActions Service Starting')
		if kodi_version() < 20:
			ok_dialog(twilight_str, 'Kodi 20 or above required[CR]Please update Kodi or uninstall Twilight')
			return logger(twilight_str, 'FirstRunActions Service Finished - Running on Incompatible Kodi Version')
		...
		if addon_version != settings_version:
			logger(twilight_str, 'FirstRunActions Running Update Actions....')
		...
		return logger(twilight_str, 'FirstRunActions Service Finished')
...
class ReuseLanguageInvokerCheck:
	def run(self):
		...
		if invoker_instance.data != current_addon_setting:
			...
			if not get_setting('twilight.auto_invoker_fix') == 'true' and not confirm_dialog(text='%s[CR]%s' % (ls(33021), ls(33020))):
...
class TraktMonitor:
	def run(self):
		logger(twilight_str, 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			try:
				...
				if status in ('success', 'no account'):
					logger(twilight_str, trakt_service_string % ('Success', success_line_dict[status]))
					if trakt_sync_refresh_widgets():
						kodi_refresh()
						logger(twilight_str, trakt_service_string % ('Widgets Refresh', 'Setting Activated. Widget Refresh Performed'))
					else: logger(twilight_str, trakt_service_string % ('Widgets Refresh', 'Setting Disabled. Skipping Widget Refresh'))
				elif status == 'failed': logger(twilight_str, trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				else: logger(twilight_str, trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
			except Exception as e: logger(twilight_str, trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del monitor
		except: pass
		try: del player
		except: pass
		return logger(twilight_str, 'TraktMonitor Service Finished')

class CustomActions:
	def run(self):
		logger(twilight_str, 'CustomActions Service Starting')
		monitor, player = xbmc_monitor(), xbmc_player()
		...
		while not abort_requested():
			...
			while not any([context_visible, info_visible]) and not abort_requested():
				...
				if not external() or get_infolabel(listitem_property_str % 'twilight.external') == 'true':
					run_custom = True
					custom_context_params = get_infolabel(listitem_property_str % 'twilight.options_params')
					custom_main_context_params = get_infolabel(listitem_property_str % 'twilight.context_main_menu_params')
					custom_info_params = get_infolabel(listitem_property_str % 'twilight.extras_params')
		...
		return logger(twilight_str, 'CustomActions Service Finished')
...
class ClearSubs:
	def run(self):
		logger(twilight_str, 'Clear Subtitles Service Starting')
		sub_formats = ('.srt', '.ssa', '.smi', '.sub', '.idx', '.nfo')
		subtitle_path = 'special://temp/%s'
		files = list_dirs(translate_path('special://temp/'))[1]
		for i in files:
			if i.startswith('TWILIGHTSubs_') or i.endswith(sub_formats): delete_file(translate_path(subtitle_path % i))
		return logger(twilight_str, 'Clear Subtitles Service Finished')

class AutoRun:
	def run(self):
		logger(twilight_str, 'AutoRun Service Starting')
		if auto_start_twilight(): run_addon()
		return logger(twilight_str, 'AutoRun Service Finished')

class PremiumExpiryCheck:
	def run(self):
		if not check_premium(): return
		logger(twilight_str, 'Premium Expiry Check Service Starting')
		self.message, self.days_remaining = [], []
		try:
			self.run_check()
			if self.make_message(): ok_dialog('Twilight - Premium Accounts Expiring...', '\n'.join(self.message))
			self.expired_premium = [i for i in self.days_remaining if i[1] == 0]
			if self.expired_premium and confirm_dialog(heading='Twilight - Premium Accounts Expired', text='Disable Expired Accounts?', ok_label=32839, cancel_label=32840): self.revoke()
		except: pass
