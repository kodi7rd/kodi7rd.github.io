...
pause_settings_prop, use_skin_fonts_prop, custom_info_prop = 'twilight.pause_settings', 'twilight.use_skin_fonts', 'twilight.custom_info_dialog'
current_skin_prop, current_font_prop = 'twilight.current_skin', 'twilight.current_font'
playback_int_prop, playback_int_run_prop = 'twilight.playback_int', 'twilight.playback_int.run'
...
def build_url(url_params):
	return 'plugin://plugin.video.twilight/?%s' % urlencode(url_params)
...
def run_addon(addon='plugin.video.twilight', block=False):
...
def disable_enable_addon(addon_name='plugin.video.twilight'):
	try:
...
def open_settings(query, addon='plugin.video.twilight'):
	hide_busy_dialog()
	if query:
		try:
...
def external_scraper_settings():
	try:
		external = get_setting('twilight.external_scraper.module', None)
