            ...
            #     numOfPages = responseJson['total_pages']
            #     if _page == 1:
            #         myLogger("Opensubtitles SearchSubtitles search result: Number of pages - %s" %(numOfPages))
            #     responseJsonData.extend(responseJson['data'])
            #     _page += 1
            #     if _page > numOfPages:
            #         break
...
'''
    def remove_brackets_content_from_text(self, text):
        ...
        start_index = new_text.rfind("(")
        end_index = new_text.rfind(")")
        if start_index != -1 and end_index != -1:
            # '(' and ')' are both present in the string
