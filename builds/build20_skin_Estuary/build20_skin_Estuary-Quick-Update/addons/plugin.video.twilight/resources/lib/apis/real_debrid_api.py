...
class RealDebridAPI:
	...
	def user_cloud(self):
		string = 'twilight_rd_user_cloud'
		url = 'torrents?limit=500'
		return cache_object(self._get, string, url, False, 0.5)

	def downloads(self):
		string = 'twilight_rd_downloads'
		url = 'downloads?limit=500'
		return cache_object(self._get, string, url, False, 0.5)

	def user_cloud_info(self, file_id):
		string = 'twilight_rd_user_cloud_info_%s' % file_id
	...
	def get_hosts(self):
		string = 'twilight_rd_valid_hosts'
		url = 'hosts/domains'
	...
	def clear_cache(self, clear_hashes=False):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT data FROM maincache WHERE id=?""", ('twilight_rd_user_cloud',))
				try: 
				...
				if not user_cloud_success:
					dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_rd_user_cloud',))
					clear_property("twilight_rd_user_cloud")
					for i in user_cloud_info_caches:
						dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_rd_user_cloud_info_%s' % i,))
						clear_property("twilight_rd_user_cloud_info_%s" % i)
					dbcon.commit()
					user_cloud_success = True
			except: user_cloud_success = False
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_rd_downloads',))
				clear_property("twilight_rd_downloads")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_rd_valid_hosts',))
				clear_property('twilight_rd_valid_hosts')
				dbcon.commit()
				dbcon.close()
