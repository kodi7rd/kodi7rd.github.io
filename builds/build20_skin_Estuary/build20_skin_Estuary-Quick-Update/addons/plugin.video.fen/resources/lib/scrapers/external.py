...
########### Settings ####################
enable_hebrew_subtitles_to_fen_sources_matching = kodi_utils.get_setting('enable_hebrew_subtitles_to_fen_sources_matching', 'false') == 'true'
#########################################
...
class source:
    ...
    def results(self, info):
        ...
        # Log the appropriate message based on the value of the enable_hebrew_subtitles_to_fen_sources_matching setting
        log_message = "Starting search_hebrew_subtitles_thread..." if enable_hebrew_subtitles_to_fen_sources_matching else "Skipping search_hebrew_subtitles_thread..."
        kodi_utils.logger("KODI-RD-IL", f"SETTING enable_hebrew_subtitles_to_fen_sources_matching is: {enable_hebrew_subtitles_to_fen_sources_matching}. {log_message}")
        
        if enable_hebrew_subtitles_to_fen_sources_matching:
            # create the search_hebrew_subtitles_thread and store it in a variable
            self.search_hebrew_subtitles_thread = thread_utils.create_search_hebrew_subtitles_thread(self.media_type, self.title, self.season, self.episode, self.year, self.tmdb_id)
            if self.search_hebrew_subtitles_thread is not None:
                self.search_hebrew_subtitles_thread_start_time = time.time()
                self.search_hebrew_subtitles_thread.start()
        #########################################
    ...
    def get_sources(self):
        ...
        ############KODI-RD-IL###################   
        # Manage search_hebrew_subtitles_thread runtime
        if enable_hebrew_subtitles_to_fen_sources_matching:
            thread_utils.manage_search_hebrew_subtitles_thread(self.search_hebrew_subtitles_thread, self.search_hebrew_subtitles_thread_start_time)
        #########################################
        ...
        try:
            for i in sources:
                try:
                    ...
                    ############KODI-RD-IL###################
                    original_fen_source_file_name = i['name']
                    #########################################
                    ...
                    # CUSTOM NEW LINE:
                    #########################################
                    i.update({'provider': provider, 'original_fen_source_file_name': original_fen_source_file_name, 'display_name': display_name, 'external': True, 'scrape_provider': self.scrape_provider, 'extraInfo': extraInfo,
                            'quality': quality, 'size_label': size_label, 'size': round(size, 2)})
                    #########################################
                except: pass
        except: pass
