        ...
        def builder(results):
            ...
            for count, item in enumerate(results, 1):
                try:
                    ...
                    if scrape_provider == 'external':
                        ...
                        try:
                            for ix,fr in name_sub:
                                ...
                                fixed_name=ix.lower().strip().replace("%20",".").replace("_",".").replace(" ",".").replace("-",".").replace(".avi","").replace(".mp4","").replace(".mkv","").replace(".aac","").replace("...",".").replace("..",".").replace(".heb","").replace(".x265","").replace(".x264","").replace("5.1.1","").replace("5.1","").replace(".h.264","").replace(".h.265","").replace(".h264","").replace(".h265","")
                                
                                fixed_name2=name.lower().strip().replace("%20",".").replace("_",".").replace(" ",".").replace("-",".").replace(".avi","").replace(".mp4","").replace(".mkv","").replace(".aac","").replace("...",".").replace("(","").replace(")","").replace("[",".").replace("]",".").replace(".x265","").replace(".x264","").replace("5.1.1","").replace("5.1","").replace(".h.264","").replace(".h.265","").replace(".h264","").replace(".h265","")
                                
                                if fixed_name in fixed_name2:
                                    subcount+=1
                                    sub='[B][COLOR deepskyblue]  SUBTITLE: [/COLOR]'+'[COLOR yellow]'+'התאמה של 100%'+'[/COLOR][/B]'
                                    break
                    ...
                    # KODI-RD-IL ADDITION - NEW CUSTOM LINE:
                    set_property('size_label', get('size_label', 'N/A') + ' ' + sub)
                    #########################################
