...
class PremiumizeAPI:
	...
	def user_cloud(self, folder_id=None):
		if folder_id:
			string = 'twilight_pm_user_cloud_%s' % folder_id
			url = 'folder/list?id=%s' % folder_id
		else:
			string = 'twilight_pm_user_cloud_root'
			url = 'folder/list'
		return cache_object(self._get, string, url, False, 0.5)

	def user_cloud_all(self):
		string = 'twilight_pm_user_cloud_all_files'
		url = 'item/listall'
		return cache_object(self._get, string, url, False, 0.5)
	...
	def get_item_details(self, item_id):
		string = 'twilight_pm_item_details_%s' % item_id
		url = 'item/details'
		data = {'id': item_id}
		args = [url, data]
	...
	def get_hosts(self):
		string = 'twilight_pm_valid_hosts'
		url = 'services/list'
	...
	def clear_cache(self, clear_hashes=True):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT id FROM maincache WHERE id LIKE ?""", ('twilight_pm_user_cloud%',))
				try:
			...
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_pm_transfers_list',))
				clear_property("twilight_pm_transfers_list")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_pm_valid_hosts',))
				clear_property('twilight_pm_valid_hosts')
				dbcon.commit()
				dbcon.close()
