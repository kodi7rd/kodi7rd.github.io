    ...
    def make_items(self, filtered_list=None):
        def builder(results):
            ...
            if enable_hebrew_subtitles_to_twilight_sources_matching:
                ...
                kodi_utils.logger("KODI-RD-IL", f"Total unique filtered subtitles found: {total_subtitles_found_count}")
            ...
            for count, item in enumerate(results, 1):
                try:
                    ...
                    if scrape_provider == 'external':
                        ...
                        ############KODI-RD-IL###################
                        # Increment the global subtitles_matched_count variable by the count returned from the function
                        # Assign the subtitle match text generated by the function to the subtitle_matches_text variable
                        if enable_hebrew_subtitles_to_twilight_sources_matching:
                        
                            original_twilight_source_file_name = get('original_twilight_source_file_name') or name
                            
                            # Sync Percentage Matching
                            matches_count, subtitle_matches_text, quality_counts_for_source = hebrew_subtitles_search_utils.calculate_highest_sync_percent_and_set_match_text(total_subtitles_found_list, original_twilight_source_file_name, quality)
