...
########### Settings ####################
enable_hebrew_subtitles_to_twilight_sources_matching = kodi_utils.get_setting('enable_hebrew_subtitles_to_twilight_sources_matching', 'false') == 'true'
#########################################
...
class source:
	def __init__(self, meta, source_dict, debrid_torrents, internal_scrapers, prescrape_sources, progress_dialog, disabled_ext_ignored=False):
		...
		self.sleep_time = 100
		self.finish_early = get_setting('twilight.search.finish.early') == 'true'
		self.timeout = 60 if disabled_ext_ignored else int(get_setting('twilight.results.timeout', '20'))
	...
	def results(self, info):
		...
		# Log the appropriate message based on the value of the enable_hebrew_subtitles_to_twilight_sources_matching setting
		log_message = "Starting search_hebrew_subtitles_thread..." if enable_hebrew_subtitles_to_twilight_sources_matching else "Skipping search_hebrew_subtitles_thread..."
		kodi_utils.logger("KODI-RD-IL", f"SETTING enable_hebrew_subtitles_to_twilight_sources_matching is: {enable_hebrew_subtitles_to_twilight_sources_matching}. {log_message}")
		...
		if enable_hebrew_subtitles_to_twilight_sources_matching:
			...
			self.search_hebrew_subtitles_thread = thread_utils.create_search_hebrew_subtitles_thread(self.media_type, self.title, self.season, self.episode, self.year, self.tmdb_id, imdb_id)
