...
class AllDebridAPI:
	...
	def get_hosts(self):
		string = 'twilight_ad_valid_hosts'
		url = 'hosts'
	...
	def clear_cache(self, clear_hashes=True):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_ad_user_cloud',))
				clear_property('twilight_ad_user_cloud')
				dbcon.commit()
				user_cloud_success = True
			except: user_cloud_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('twilight_ad_valid_hosts',))
				clear_property('twilight_ad_valid_hosts')
				dbcon.commit()
				dbcon.close()
