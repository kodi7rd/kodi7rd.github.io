                ...
                if "MkEWBc" in decoded_line:
                    try:
                        ...
                        if len(response) == 1:
                            ...
                            n_text=[]
                            for items in translate_text.split('\n'):
                                t_item=items.strip()
                                log.warning('LineB:'+t_item)
                                if t_item.startswith("."):
                                    n_text.append(t_item[:])
                                    log.warning('Start With')
                                elif  t_item.endswith("."):
                                    n_text.append(t_item[:-1])
                                    log.warning('Ends With')
                                else:
                                    n_text.append(t_item)
                                    log.warning('Ok')
                                log.warning('Line:'+t_item)
                            translate_text='\n'.join(n_text).replace('->','-->')
                            if pronounce == False:
