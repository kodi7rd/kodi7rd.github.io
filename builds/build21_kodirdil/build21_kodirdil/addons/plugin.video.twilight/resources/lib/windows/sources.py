	...
	def make_items(self, filtered_list=None):
		def builder(results):
			...
			if enable_hebrew_subtitles_to_twilight_sources_matching:
				...
				global total_external_subtitles_found_count
				global total_hebrew_embedded_subtitles_matches_count
				global total_subtitles_matches_count
				global total_quality_counts

				total_external_subtitles_found_count = len(total_subtitles_found_list)
				total_hebrew_embedded_subtitles_matches_count = 0
				total_subtitles_matches_count = 0

				# Log the total subtitles found count
				kodi_utils.logger("KODI-RD-IL", f"Total unique filtered subtitles found: {total_external_subtitles_found_count}")
			...
			for count, item in enumerate(results, 1):
				try:
					...
					source_from_cloud_text=''

					if '_cloud' in scrape_provider:
						source_from_cloud_text = "[B][COLOR orange]הופעל בעבר[/COLOR][/B] | "

					# Increment the global total_subtitles_matches_count variable by the count returned from the function
					# Assign the subtitle match text generated by the function to the subtitle_matches_text variable
					if enable_hebrew_subtitles_to_twilight_sources_matching:

						if scrape_provider == 'external':
							# original_twilight_video_tagline is set in external.py - process_sources function
							original_twilight_video_tagline = get('original_twilight_video_tagline') or name
						else:
							# original_twilight_video_tagline is set to get 'name' param from AD/PM/RD cloud (original TagLine before cleaning)
							original_twilight_video_tagline = get('name') or name

						# Sync Percentage Matching
						external_subtitles_matched_count, hebrew_embedded_subtitles_matched_count, subtitle_matches_text, quality_counts_for_source = hebrew_subtitles_search_utils.calculate_highest_sync_percent_and_set_match_text(total_subtitles_found_list, original_twilight_video_tagline, quality, hebrew_embedded_taglines)
						total_subtitles_matches_count += external_subtitles_matched_count + hebrew_embedded_subtitles_matched_count
						total_hebrew_embedded_subtitles_matches_count += hebrew_embedded_subtitles_matched_count
					...
					set_property('extraInfo', source_from_cloud_text + extraInfo)
					#########################################
...
class SourcesPlayback(BaseDialog):
	...
	def set_scraper_properties(self):
		...
		self.setProperty('flag_highlight', self.get_setting('twilight.scraper_flag_identify_colour', 'FF7C7C7C'))
		self.setProperty('result_highlight', self.get_setting('twilight.scraper_result_identify_colour', 'FFFFFFFF'))
