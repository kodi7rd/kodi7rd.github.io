...
class EpisodeTools:
	...
	############KODI-RD-IL###################
	def get_next_episode_info_for_skin_next_episode_button(self):
		try:
			# Retrieving current settings]
			current_date = get_datetime()  # Getting the current date and time
			# Retrieving current season and episode
			season_data = self.meta_get('season_data')
			current_season, current_episode = int(self.meta_get('season')), int(self.meta_get('episode'))
			# Determining next season/episode
			curr_season_data = [i for i in season_data if i['season_number'] == current_season][0]
			season_number_for_next_episode_button = current_season if current_episode < curr_season_data['episode_count'] else current_season + 1
			episode_number_for_next_episode_button = current_episode + 1 if current_episode < curr_season_data['episode_count'] else 1
			# Fetching episode data
			ep_data = episodes_meta(season_number_for_next_episode_button, self.meta, settings.metadata_user_info())
			# Checking for valid episode data
			if not ep_data: return 0, 0
			try: ep_data = [i for i in ep_data if i['episode'] == episode_number_for_next_episode_button][0]
			except: return 0, 0
			# Parsing episode air date
			airdate = ep_data['premiered']
			d = airdate.split('-')
			episode_date = date(int(d[0]), int(d[1]), int(d[2]))
			# Checking if the episode has aired yet
			if current_date < episode_date: return 0, 0
			# Returning the season and episode number of the next current episode
			return season_number_for_next_episode_button, episode_number_for_next_episode_button
		except:
			return 0, 0  # Error occurred during the process or no next episode available
	#########################################
