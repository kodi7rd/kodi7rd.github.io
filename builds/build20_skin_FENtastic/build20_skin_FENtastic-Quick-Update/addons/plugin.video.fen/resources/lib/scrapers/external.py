...
class source:
    ...
    # KODI-RD-IL ADDITION - NEW CUSTOM FUNCTION:  
    def check_subtitle(self,tv_movie,original_title,name,season,episode,show_original_year,id): 
            from modules import subs
            ...
            f_subs=''
            try:
                f_subs=sub_cache.get(subs.get_links,24,tv_movie,original_title,name,season_n,episode_n,season,episode,show_original_year,id,check_one=True, table='posters')
            except:pass
    ...
    def get_movie_source(self, provider, module):
        ...
        try:
            t = Thread(target=self.check_subtitle, args=('movie',self.title,self.title,'0','0',self.year,self.tmdb_id,))
            t.start()
        except:pass
        #########################################
    ...
    def get_episode_source(self, provider, module, pack):
        ...
        # KODI-RD-IL ADDITION - NEW CUSTOM LINES:
        try:
            t = Thread(target=self.check_subtitle, args=('tv',self.title,self.title,self.season,self.episode,self.year,self.tmdb_id,))
            t.start()
        except:pass
        #########################################
