...
def window_manager(obj):
	def close():
		...
		clear_property('twilight.param_stack')
	def monitor():
		timer = 0
	...
	def get_stack():
		try: param_stack = json.loads(get_property('twilight.param_stack'))
		except: param_stack = []
		return param_stack
	def add_to_stack(params):
		param_stack.append(params)
		set_property('twilight.param_stack', json.dumps(param_stack))
